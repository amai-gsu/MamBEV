...
# ---------------------------------------------
#  Updated to mmcv 2.0 by Jack Morris
# ---------------------------------------------
...
class SpatialCrossAttention(BaseModule):
    ...
    def __init__(
        ...
        deformable_attention: Optional[Dict] = dict(
    ...
    def forward(
        ...
        **kwargs,
        ...
        slots = torch.zeros_like(inp_residual)
        ...
        # feat_flatten = rearrange(feat_flatten, "num_cam bs hw dim -> num_cam hw bs dim")
        value = rearrange(value, "bs num_cams layers dim -> (bs num_cams) layers dim")
...
class MSDeformableAttention3D(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ), f"Values are not compatible with spatial shapes given\n{spatial_shapes}\n{(spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum()} != {num_value}"
        ...
        if reference_points.shape[-1] == 2:
            """
            For each BEV query, it owns `num_Z_anchors` in 3D space that have different heights.
        ...
        assert (
            output is not None
        ), "Unexpected output from MultiScaleDeformableAttn Function"
        if not self.batch_first:
