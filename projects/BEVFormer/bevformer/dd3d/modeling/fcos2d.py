...
class FCOS2DHead(nn.Module):
    def __init__(
        self,
        num_classes: int,
        input_shape: Sequence[ShapeSpec],
        num_cls_convs: int = 4,
        num_box_convs: int = 4,
        norm: str = "BN",
        use_scale: bool = True,
        box2d_scale_init_factor: float = 1.0,
        version: int = 2,
    ):
        super().__init__()
        ...
        assert None not in set(self.in_strides), "Invalid stride in input"
        ...
        assert in_channels is not None, "Must have a valid number of channels"
        ...
        if self.use_scale:
            if self._version == 1:
                self.scales_reg = nn.ModuleList(
                    [
                        Scale(init_value=stride * self.box2d_scale_init_factor)  # type:ignore
    ...
    def forward(self, x: Sequence[torch.Tensor]):
        ...
        for i, feature in enumerate(x):
            ...
            if self.use_scale:
                ...
                if self._version == 1:
                    box_reg = self.scales_reg[i](box_reg)
                else:
                    box_reg = self.scales_box2d_reg[i](box_reg)
