...
from ..datasets.nuscenes import MAX_NUM_ATTRIBUTES
...
class NuscenesDD3D(DD3D):
    def __init__(
        ...
        in_channels,
        strides,
        fcos2d_cfg: Dict = dict(),
        fcos2d_loss_cfg: Dict = dict(),
        fcos3d_cfg: Dict = dict(),
        fcos3d_loss_cfg: Dict = dict(),
        target_assign_cfg: Dict = dict(),
        nusc_loss_weight: Dict = dict(),
        box3d_on: bool = True,
    ...
    def forward(
        self, features: List[torch.Tensor], batched_inputs: Sequence[OrderedDict]
    ):
        assert isinstance(
            features, Sequence
        ), f"Expected features to be Sequence instead got {type(features)}"
        assert isinstance(
            features[0], torch.Tensor
        ), f"Expected features to be Sequence of Tensor instead got {type(features[0])}"
        assert isinstance(
            batched_inputs, Sequence
        ), f"Expected features to be Sequence instead got {type(batched_inputs)}"
        assert isinstance(
            batched_inputs[0], OrderedDict
        ), f"Expected features to be Sequence of OrderedDict instead got {type(batched_inputs[0])}"

        # print(tuple(batched_inputs[0].keys()))
        # exit()
