...
class FCOS3DHead(nn.Module):
    ...
    def _init_weights(self):
        for layer in self.box3d_tower.modules():
        ...
        for modules in predictors:
            for layer in modules.modules():
                if isinstance(layer, nn.Conv2d):
                    ...
                    if layer.bias is not None:  # depth head may not have bias.
                        torch.nn.init.constant_(layer.bias, 0)
    ...
    def forward(self, x: Sequence[torch.Tensor]):
        ...
        for layer, features in enumerate(x):
            ...
            _l = layer if self.use_per_level_predictors else 0
            ...
            if self.use_scale:
                ...
                proj_ctr = self.scales_proj_ctr[layer](proj_ctr)
                size3d = self.scales_size[layer](size3d)
                conf3d = self.scales_conf[layer](conf3d)
                depth = self.offsets_depth[layer](self.scales_depth[layer](depth))
...
class FCOS3DLoss(nn.Module):
    ...
    def forward(
        self,
        box3d_quat: torch.Tensor,
        box3d_ctr: torch.Tensor,
        box3d_depth: torch.Tensor,
        box3d_size: torch.Tensor,
        box3d_conf: torch.Tensor,
        dense_depth,
