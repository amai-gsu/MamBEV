...
from mmdet.models.layers import NormedLinear, inverse_sigmoid
...
from ..task_utils.utils import normalize_bbox
from .petr_head import pos2posemb3d
...
class StreamPETRHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        match_with_velo: bool = True,
        code_weights: Optional[List[float]] = None,
        match_costs: Optional[List[float]] = None,
        ...
        depth_step: float = 0.8,
        depth_num: int = 64,
        LID: bool = False,
        depth_start: int = 1,
        position_range: List[float] = [-65.0, -65.0, -8.0, 65.0, 65.0, 8.0],
        scalar: int = 5,
        noise_scale: float = 0.4,
        noise_trans: float = 0.0,
        dn_weight: float = 1.0,
        split: float = 0.5,
        init_cfg: Optional[MultiConfig] = None,
        normedlinear: bool = False,
        **kwargs,
    ):
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            assert self.memory_velo is not None
    ...
    def post_update_memory(
        ...
        assert isinstance(self.memory_reference_point, torch.Tensor)
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        if self.with_ego_pos:
            ...
            assert isinstance(self.memory_velo, torch.Tensor)
            assert isinstance(self.memory_timestamp, torch.Tensor)
            assert isinstance(self.memory_egopose, torch.Tensor)
        ...
        if self.num_propagated > 0:
            assert isinstance(temp_memory, torch.Tensor)
    ...
    def prepare_for_dn(
        self, batch_size: int, reference_points: torch.Tensor, img_metas
    ):
    ...
    def get_targets(
        ...
        gt_bboxes_ignore_list: Optional[List[Optional[torch.Tensor]]] = None,
    ):
