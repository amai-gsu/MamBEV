# Code from torchviz package - patched to work with torch 2.0+
...
class ComputeGraphVisHook(Hook):
    def __init__(
        self,
        model_name: str,
        target_output_key: Union[str, int],
        show_attrs: bool,
        save_dir: str,
        show_saved: bool,
        output_file_format: str = "png",
        graph_renderer: Optional[str] = None,
        start_offset: int = 0,
        active_iters: int = 1,
    ) -> None:
        ...
        self.target_output = target_output_key
        self.inactive = False
    ...
    def __call__(self, output: torch.Tensor, params: Dict) -> Any:
        self._batch_idx += 1
        if self.start_offset > self._batch_idx:
            return
        ...
        print_log(
            f"Saved compute graph to {self.save_dir.absolute()/( self.model_name + '.'+ self.output_file_format )}",
            "current",
            logging.INFO,
        )
    ...
    def _after_iter(
        ...
        if mode != "train":
            return
        if self.start_offset > batch_idx:
            return
        if self.active_iters <= 0:
            return
        ...
        model_name = Path(runner.work_dir).stem
        assert outputs is not None
        assert isinstance(outputs, dict) or isinstance(self.target_output, int)
        target_out = outputs[self.target_output]  # type:ignore
        make_dot(
            target_out,
            dict(runner.model.named_parameters()),
            show_attrs=self.show_attrs,
            show_saved=self.show_saved,
        ).render(
            model_name,
