...
def update_nuscenes_infos(pkl_path:str, out_dir:str):
    camera_types = [
        "CAM_FRONT",
        "CAM_FRONT_RIGHT",
        "CAM_FRONT_LEFT",
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list["infos"])):
        ...
        temp_data_info["lidar_points"]["num_pts_feats"] = ori_info_dict.get( #type:ignore
            "num_features", 5
        )
        temp_data_info["lidar_points"]["lidar_path"] = Path( #type:ignore
            ori_info_dict["lidar_path"]
        ).name
        temp_data_info["lidar_points"]["lidar2ego"] = convert_quaternion_to_matrix( #type:ignore
        ...
        for ori_sweep in ori_info_dict["sweeps"]:
            ...
            temp_lidar_sweep["lidar_points"]["lidar2ego"] = ( #type:ignore
                convert_quaternion_to_matrix(
                    ori_sweep["sensor2ego_rotation"],
                    ori_sweep["sensor2ego_translation"],
                )
            )
            temp_lidar_sweep["ego2global"] = convert_quaternion_to_matrix( #type:ignore
            ...
            temp_lidar_sweep["lidar_points"]["lidar2sensor"] = lidar2sensor.astype( #type:ignore
                np.float32
            ).tolist()
            temp_lidar_sweep["timestamp"] = ori_sweep["timestamp"] / 1e6
            temp_lidar_sweep["lidar_points"]["lidar_path"] = ori_sweep["data_path"] #type:ignore
            temp_lidar_sweep["sample_data_token"] = ori_sweep["sample_data_token"]
            temp_data_info["lidar_sweeps"].append(temp_lidar_sweep) #type:ignore
        temp_data_info["images"] = {}
        for cam in ori_info_dict["cams"]:
            empty_img_info = get_empty_img_info()
            empty_img_info["img_path"] = Path( #type:ignore
            ...
            empty_img_info["cam2ego"] = convert_quaternion_to_matrix( #type:ignore
        ...
        if "gt_boxes" in ori_info_dict:
            ...
            for i in range(num_instances):
                ...
                if ori_info_dict["gt_names"][i] in METAINFO["classes"]:
                    empty_instance["bbox_label"] = METAINFO["classes"].index( #type:ignore
                        ori_info_dict["gt_names"][i]
                    )
                else:
                    ignore_class_name.add(ori_info_dict["gt_names"][i])
                    empty_instance["bbox_label"] = -1 #type:ignore
                empty_instance["bbox_label_3d"] = copy.deepcopy( #type:ignore
        ...
        if "pts_semantic_mask_path" in ori_info_dict:
            temp_data_info["pts_semantic_mask_path"] = Path( #type:ignore
...
def update_pkl_infos(dataset:str, out_dir:str, pkl_path:str):
